...
TESTED_LINE="Car1 snow is too deep in Lane1_1, checking other lanes"
...
TESTED_LINE="Car1 no other suitable lane found, stuck in snow"
if grep -q "$TESTED_LINE" "$OUTPUT_FILE"; then
:
else
	SUCCESS=false
...
TESTED_LINE="INFO Car1 is stuck"
if grep -q "$TESTED_LINE" "$OUTPUT_FILE"; then
:
else
	SUCCESS=false
...
TESTED_LINE="INFO Car1 current lane is Lane1_1"
if grep -q "$TESTED_LINE" "$OUTPUT_FILE"; then
:
else
	SUCCESS=false
